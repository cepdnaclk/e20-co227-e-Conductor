import { Box, Button, Checkbox, FormControl, FormControlLabel, FormGroup, FormLabel, Grid, Paper} from '@mui/material'
import React, { useEffect, useState } from 'react'
import Stepper from '../../ProgressBars/Stepper';


export default function BusFilter({ steps, activeStep, handleBack,  setServiceFilter, setRouteFilter, setOrgFilter}) {
  ...
  const handleService = (event) => {
    setServiceItems({...serviceItems, [event.target.value]: event.target.checked });
  };

  // Handling route items
  const handleRoute = (event) => {
    setRouteItems({...routeItems, [event.target.value]: event.target.checked });
  };
  ...
  return (
    <Paper sx={{
      ...
      <Box sx={{width:'100%', height:'calc(100% - 100px)', display:'flex', flexDirection:'column', justifyContent:'space-between'}}>
        <Box  sx={{display:'flex', flexDirection:'column', gap:'20px'}}>
          <FormControl component="fieldset">
            ...
            <FormGroup aria-label="position" row sx={{display:'flex' , justifyContent:'space-between', width:'100%'}}>
              <FormControlLabel
                value="super luxury"
                control={<Checkbox checked={serviceItems.super} onChange={handleService} />}
                label="Super Luxury"
                labelPlacement="end"
              />
              <FormControlLabel
                value="luxury"
                control={<Checkbox checked={serviceItems.luxury} onChange={handleService} />}
                label="Luxury"
                labelPlacement="end"
              />
              <FormControlLabel
                value="semi luxury"
                control={<Checkbox checked={serviceItems.semi} onChange={handleService} />}
                label="Semi Luxury"
                labelPlacement="end"
              />
              <FormControlLabel
                value="normal"
                control={<Checkbox checked={serviceItems.normal} onChange={handleService} />}
                label="Normal"
                labelPlacement="end"
              />
            </FormGroup>
          ...
          <FormControl component="fieldset" fullWidth >
            ...
            <Grid container>
              <Grid item xs={6}>
                <FormControlLabel
                  value="expressway"
                  control={<Checkbox checked={routeItems.express} onChange={handleRoute} />}
                  label="Expressway"
                  labelPlacement="end"
                />
                </Grid>
                ...
                <FormControlLabel
                  value="normalway"
                  control={<Checkbox checked={routeItems.normal} onChange={handleRoute} />}
                  label="Normalway"
                  labelPlacement="end"
                />
                </Grid>
            ...
            <FormGroup aria-label="position" row sx={{display:'flex' , width:'100%'}}>
              <Grid container>
                <Grid item xs={6}>
                  <FormControlLabel
                    value="sltb"
                    control={<Checkbox checked={orgItems.sltb} onChange={handleOrg} />}
                    label="SLTB"
                    labelPlacement="end"
                  />
                </Grid>
                <Grid item xs={6}>
                  <FormControlLabel
                    value="privert"
                    control={<Checkbox checked={orgItems.pvt} onChange={handleOrg} />}
                    label="Privert"
                    labelPlacement="end"
                  />
                </Grid>
        ...
        <Box>
          <Button variant='outlined' fullWidth onClick={handleBack}>Back</Button>
        </Box>
      </Box>
    </Paper>
  );
