import { Box, Button, ButtonGroup, Card, Grid, IconButton, Paper } from '@mui/material'
...
export default function Earnings({setLoading, language}) {
  ...
  const getGeneral = async() => {
    try {
      ...
      const serverResponse = await GetRequest(userID, 'income/general');
  ...
  const getSummary = async(value) => {
    ...
    try {
      const serverResponse = await GetRequest(data, 'income/summary');
  ...
  const getIncome = async() => {
    ...
    try {
      const serverResponse = await GetRequest(data, 'income/income');
      //console.log('Income Data Income: ', serverResponse.data);
      setIncome({loaded:true, data:serverResponse.data.graphData, earning: serverResponse.data.earning});
    } catch (error) {
      console.log('Error in fetching general data!');
    }
  };
      ...
      {general.loaded && 
        <Grid container spacing={2} mb={2} minWidth={'390px'}>
          ...
          <Grid item xs={12} md={6} lg={5} textAlign={'center'} p={2} pb={0}>
            {income.loaded ? 
              <Card sx={{width:'100%', height:'100%', padding:'20px 0', borderRadius:'12px', display:'flex', flexDirection:'column', justifyContent:'start', alignItems:'center', gap:'20px'}}>
                <Box display={'flex'} alignItems={'center'} gap={1}>
                  <Texts fontColor='textSecondary' variant={'h6'}>
                    {`${TODAY.getFullYear()}-${String(TODAY.getMonth()+1).padStart(2, '0')}-${String(TODAY.getDate()).padStart(2, '0')} | ${getDay(TODAY.getDay())}`}
                  </Texts>
                ...
                <PieChartWithCenterLabel
                  label={`Earning (LKR)\n${parseInt(income.earning).toLocaleString()}`}
                  data={income.data}
                  rows={rows}
                />              
              </Card> 
          ...
          {/* Summary */}
          <Grid item xs={12} p={2}>
            <Card sx={{padding:'20px', width:'100%', borderRadius:'12px', cursor:'pointer', minHeight:'120px'}}>
              <Grid container spacing={2}>
                ...
                <Grid item xs={12} md={6} textAlign={{xs:'center', md:'end'}}>
                  <ButtonGroup variant="outlined" sx={{height:'30px'}}>
                    <Button onClick={handleChange} id='year' >Year</Button>
                    <Button onClick={handleChange} id='month' >Month</Button>
                    <Button onClick={handleChange} id='week' >Week</Button>
                  </ButtonGroup>
                </Grid>
              </Grid>

              <Box width={'100%'} ref={containerRef} overflow={'auto'} display={'flex'} justifyContent={'space-around'} height={'max-content'}>
                {summary.loaded ? 
                  <LineChart
                    series={summary.data}
                    slotProps={{
                  ...
                  /> : <Skeleton sx={{width: '340px', height:'28px', mt:'20px', borderRadius:'12px'}}/>
                }
              </Box>
            </Card> 
          </Grid>
