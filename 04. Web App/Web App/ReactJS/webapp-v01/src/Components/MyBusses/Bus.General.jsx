...
function BusCard({ details, handleDelete, handleEdit, handleDetails }) {
  ...
  const today = useDateAndTime().date;
  const available = details.insuranceExp > today && details.VRL_Exp > today;
      ...
      {available ? (
        ...
        >
          <CheckCircleOutlineIcon
            sx={{ fontSize: "15px", color: "#04aa6d", mr: "5px" }}
          />
          <Texts variant="caption" fontColor="#04aa6d">
            {" "}
            Bus is ready to go.{" "}
          </Texts>
        </Box>
        ...
        >
          <WarningAmberIcon
            sx={{ fontSize: "15px", color: "#cc3300", mr: "5px" }}
          />
          <Texts variant="caption" fontColor="#cc3300">
            {" "}
            Licence / insurance was expired.{" "}
          </Texts>
        </Box>
      )}
        ...
        >
          ...
          >
            ...
            >
              ...
              <Texts variant={"h6"} fontColor="#66666d">
                Total Rides
              </Texts>
            </Box>
          </Grid>
...
export default function BusGeneral({ language, setLoading }) {
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      try {
        ...
        const serverResponse = await getData("bus/info", data);
    ...
    if (selected.length > 0) {
      switch (action) {
        ...
        case "track": {
          ...
          const busList = buses.data.map((bus) => ({
            id: bus.id,
            regNo: bus.regNo,
          }));
          sessionStorage.setItem("TrackingList", JSON.stringify(selected));
          sessionStorage.setItem("MyBuses", JSON.stringify(busList));
  ...
  const deleteBus = async (dataList) => {
    try {
      setLoading(true); // Enabling spinner
      const serverResponse = await deleteData("bus/info", dataList);
  ...
  const saveBus = async (data) => {
    try {
      setLoading(true); // Enabling spinner
      const serverResponse = await postData("bus/info", data);
      ...
      if (serverResponse.data === "success") {
        ...
        ToastAlert({
          type: "error",
          title:
            "The bus is alredy exist in the system! Please check again or contact us.",
          timer: 4000,
        });
      }
    } catch (error) {
  ...
  const updateBus = async (data) => {
    try {
      setLoading(true); // Enabling spinner
      const serverResponse = await updateData("bus/info", data);
            ...
            <TableRow>
              <TableCell align="center">
                <Checkbox
                  color="primary"
                  indeterminate={
                    selected.length > 0 && selected.length < busCount
                  }
