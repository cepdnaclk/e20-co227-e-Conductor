      ...
      >
        ...
        >
          ...
          <Menu
            ...
            onClose={handleClose}
          >
            <MenuItem id={null} onClick={handleClose}>
              <em>None</em>
            </MenuItem>
            {filterList.map((item, index) => (
              <MenuItem key={index} id={item} onClick={handleClose}>
                {" "}
                {item}{" "}
              </MenuItem>
            ))}
...
export default function CustomTable({
  ...
  React.useEffect(() => {
    setIsHidden(
      button1Label !== undefined || button2Label !== undefined ? false : true
    );
  ...
  if (bodyData.length > 0 && bodyData[0].id !== null) {
    filteredRows = bodyData.filter((row) => {
      ...
      const matchesFilter = filter
        ? row[filterColumn].toLowerCase().includes(filter.toLowerCase())
        : true;
