...
export default function AvailableTickets({language, setLoading}) {
  ...
  const handleCancel = (refNo) => {
    ...
    requestRefund({
      cancelDate: `${date.getFullYear()}-${date.getMonth() + 1}-${date.getDate()}`,
  ...
  const confirmRefund = async(values) => {
    ...
    try {
      ...
      if (serverResponse.data === 'success') {
        ToastAlert({
          type: 'success',
          title: 'Refund is successful. Please check your email to more details.',
          onClose: refresh
        });
      } else {
        ToastAlert({
        ...
        <Grid container spacing={2} display={'flex'} justifyContent={'space-around'} height={'100%'}>
          {tickets.length > 0 ? tickets.map(ticket => 
            ...
            <Box height={'100%'} width={'100%'} display={'flex'} margin={'50px 0'} flexDirection='column' alignItems={'center'}> 
              ...
              <Texts fontColor='textSecondary' fontSize='30px'>No tickets available!</Texts>
            </Box>
          )}
        </Grid>
      </Box>

      <ConfirmBox open={open} data={formData} handleClose={()=>setOpen(false)} handleAgree={handleAgree}  />
    </Box>
  )
}